...
import {pair_contract} from '../contracts/common-fi/pair_contract';
    ...
    for (const ex of signedBlock.block.extrinsics) {
      ...
      if (isSigned) {
        ...
        if (
          ...
          let decodedMessage = inw_contract.abi.decodeMessage(
            compactAddLength(hexToU8a(args?.data)),
          );
          const {identifier, method, path} = decodedMessage.message;
          if (
            !(identifier === 'PSP22::transfer' && method === 'psp22::transfer')
          ) {
            continue;
          }
          if (decodedMessage?.args) {
        ...
        const filter = {
          ...
          tokenAddress: newData.tokenContract
            ? newData.tokenContract.toString()
            : '',
          amount: newData.amount
            ? convertNumberWithoutCommas(newData.amount)
            : 0,
        };
...
export const fetchInwPrice = async (pairAddress: string) => {
  ...
  if (gasLimit) {
    try {
      const {result, output} = await contract.query['pair::getReserves'](
      ...
      if (result.isOk) {
        ...
        const azeroAmount = ret[0]?.replace(/,/g, '') / Math.pow(10, 12);
